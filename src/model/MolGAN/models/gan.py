...
from src.others.MolGAN.models import postprocess_logits
from src.others.MolGAN.utils.layers import multi_dense_layers
...
class GraphGANModel(object):
    ...
    def __init__(self, vertexes, edges, nodes, embedding_dim, decoder_units, discriminator_units,
        ...
        self.dropout_rate = tf.compat.v1.placeholder_with_default(0., shape=())
        self.soft_gumbel_softmax = tf.compat.v1.placeholder_with_default(soft_gumbel_softmax, shape=())
        self.hard_gumbel_softmax = tf.compat.v1.placeholder_with_default(hard_gumbel_softmax, shape=())
        ...
        self.rewardR = tf.compat.v1.placeholder(dtype=tf.float32, shape=(None, 1))
        self.rewardF = tf.compat.v1.placeholder(dtype=tf.float32, shape=(None, 1))
        self.adjacency_tensor = tf.compat.v1.one_hot(self.edges_labels, depth=edges, dtype=tf.float32)
        self.node_tensor = tf.compat.v1.one_hot(self.nodes_labels, depth=nodes, dtype=tf.float32)
    ...
    def D_x(self, inputs, units):
        with tf.compat.v1.variable_scope('discriminator', reuse=tf.compat.v1.AUTO_REUSE):
            ...
            outputs = tf.compat.v1.layers.dense(outputs1, units=1)
